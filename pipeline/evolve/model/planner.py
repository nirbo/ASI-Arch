...
- **Keep Class Name**: Always keep class name as BasicTransformer
- **Maintain Decorators**: Keep @torch.compile decorators for performance where applicable
...
## Target: Replace Quadratic Attention
The current BasicTransformer uses standard O(N²) self-attention. Your goal is to replace this with:
- Linear attention mechanisms (kernel methods, random features)
- State-space models (Mamba, S4, etc.)
- Recurrent architectures with linear complexity
- Sparse attention patterns
- Any novel sub-quadratic attention mechanism
...
- **name**: Model identifier starting with "transformer_" (string)
- **motivation**: Clear explanation of WHAT you implemented and WHY (string)

IMPORTANT: Your response must be valid JSON format like:
{"name": "transformer_example", "motivation": "explanation here"}

Do NOT use markdown formatting, asterisks, or any other formatting in your response.""",
...
## Output Requirements
You MUST return a valid JSON object with exactly these fields:
- **name**: Model identifier starting with "delta_net_" (string)
- **motivation**: Clear explanation of WHAT you implemented and WHY (string)

IMPORTANT: Your response must be valid JSON format like:
{"name": "delta_net_example", "motivation": "explanation here"}
